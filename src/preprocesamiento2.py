...
def extract_features(audioNormalizado):
    ...
    # tonnet = librosa.feature.tonnetz(y=audio, sr=fs, chroma= chroma)
    # tonnetM = np.mean(tonnet.T, axis=0)
...
if __name__ == "__main__":
    with m as source:
        try:
            ...
            # ! GOOGLE RECOGNIZER
            # palabraGoogle = r.recognize_google(audio, language="es-CO")
            # print(palabraGoogle)
            # predicion, = modelo_entrenado.predict(palabraGoogle) #.argmax(axis=0)
            # y_prob = modelo_entrenado.predict_proba(palabraGoogle)
            # print(predicion)
            # print(y_prob)
